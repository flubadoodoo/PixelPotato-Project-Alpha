...
public class GameplayState extends BasicGameState {
	...
	private void handleCollisions(Rectangle2D boundingBox, Tile[] tiles, int delta) {
			...
			if (isOnScreen (tile)) {
				if (boundingBox.intersects (tile.getBoundingBox ())) {
					...
					if (overlap.getWidth () > overlap.getHeight ()) {
						...
						player.setyVel (0.0);
						fall = false;
						if (player.getMovementState () == Player.PLAYER_MOVEMENT_STATE.Jumping) player.setMovementState (Player.PLAYER_MOVEMENT_STATE.Standing);
					} else {
		...
		if (fall) {
			...
			if (player.getMovementState () != Player.PLAYER_MOVEMENT_STATE.Jumping) player.setMovementState (Player.PLAYER_MOVEMENT_STATE.Jumping);
		}
	}
	...
	private void handleInput(Input input, GameContainer gc, StateBasedGame game, int delta) throws SlickException {
		...
		if (input.isKeyDown (Input.KEY_D)) {
			...
			if (player.getWalkingState () != Player.PLAYER_WALKING_STATE.Right) player.setWalkingState (Player.PLAYER_WALKING_STATE.Right);
		}
		if (input.isKeyDown (Input.KEY_A)) {
			player.incrementxVel (MOVE_SPEED * delta);
			if (player.getWalkingState () != Player.PLAYER_WALKING_STATE.Left) player.setWalkingState (Player.PLAYER_WALKING_STATE.Left);
		}
		if (!input.isKeyDown (Input.KEY_D) && !input.isKeyDown (Input.KEY_A)) {
			player.setxVel (player.getxVel () * 0.89);
			if (player.getxVel () < 0.001 && player.getxVel () > -0.001) player.setxVel (0.0);
		}
